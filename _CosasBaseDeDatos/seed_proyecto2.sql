-- proyecto2_seed_minimal_ascii.sql
-- Inserta 2 filas por cada tabla del esquema `Proyecto2` sin tildes ni caracteres especiales.
-- Nota: reutilizamos los mismos 2 usuarios como Medicos, Farmaceutas y Administradores
-- para cumplir "2 datos por tabla" sin anadir usuarios extra.
...
INSERT INTO `Medico` (`nombre`, `especialidad`, `id`) VALUES
  ('Dra. Laura Gomez', 'Medicina General', 'U1'),
  ('Dr. Andres Vega',  'Pediatria',        'U2')
ON DUPLICATE KEY UPDATE id = VALUES(id);

-- 3) PACIENTE (2)
INSERT INTO `Paciente` (`id`, `nombre`, `telefono`, `fechaNacimiento`) VALUES
  ('P001', 'Ana Lopez',    '8888-0001', '1992-03-14'),
  ('P002', 'Carlos Mendez','8888-0002', '1989-07-22')
ON DUPLICATE KEY UPDATE id = VALUES(id);

-- 4) MEDICAMENTO (2)
INSERT INTO `Medicamento` (`codigo`, `nombre`, `presentacion`) VALUES
  ('MED001', 'Ibuprofeno',  'Tabletas 200 mg'),
  ('MED002', 'Amoxicilina', 'Capsulas 500 mg')
...
-- OJO: `Prescripcion`.`id` NO es AUTO_INCREMENT, asi que asignamos IDs unicos manualmente.
INSERT INTO `Prescripcion` (`id`, `indicaciones`, `duracion`, `cantidad`, `receta`, `medicamento`) VALUES
  (1001, '1 tableta cada 8 horas despues de comidas', 5, 15, @r1, 'MED001'),
  (1002, '1 capsula cada 12 horas',                    7, 14, @r2, 'MED002')
ON DUPLICATE KEY UPDATE id = VALUES(id);
